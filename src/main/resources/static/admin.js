...
async function initOverview() {
  try {
    ...
    [
      ...
      ["pending-teachers", summary.pendingTeachers],
      ["total-courses", summary.totalCourses],
      ["pending-courses", summary.pendingCourses],
      ["total-enrollments", summary.totalEnrollments],
      ["total-quizzes", summary.totalQuizzes],
    ].forEach(([id, value]) => {
      const metric = document.querySelector(`#${id}`);
      metric.textContent = value;
      metric.closest("article")?.classList.toggle("is-zero", Number(value) === 0);
    });
  } catch (error) {
    showMessage(`Could not load dashboard statistics: ${error.message}`);
  }
}
...
async function initReports() {
  ...
  try {
    ...
    summaryMetrics.forEach(([label, value]) => {
      ...
      card.append(node("p", "", label), node("strong", "", value), bar);
      summary.append(card);
    });
